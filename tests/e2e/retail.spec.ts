...
const P2TR_ADDR = 'bc1p5d7rjq7g6rdk2yhzks9smlaqtedr4dekq08ge8ztwac72sfr9rusxg3297' // P2TR
...
test.describe('Retail scan flow', () => {
  ...
  test('page renders address input and submit button', async ({ page }) => {
    await expect(page.getByRole('textbox', { name: /bitcoin address/i })).toBeVisible()
    await expect(page.getByRole('button', { name: /scan address/i })).toBeVisible()
  })

  test('shows inline validation error for garbage input without network call', async ({ page }) => {
  ...
  test('shows inline validation error for empty submission', async ({ page }) => {
    await page.getByRole('button', { name: /scan address/i }).click()
    await expect(page.getByRole('alert')).toBeVisible()
  })

  test('scans genesis address and shows SAFE_AT_REST result', async ({ page }) => {
    await page.getByRole('textbox').fill(GENESIS)
    await page.getByRole('button', { name: /scan address/i }).click()
    // Wait up to 15s for API response
    await expect(page.getByText(/safe at rest/i)).toBeVisible({ timeout: 15_000 })
  ...
  test('scans a P2TR address', async ({ page }) => {
    ...
    await expect(
      page.getByText(/exposed|empty/i).first()
