...
describe('EmptyCard', () => {
  it('renders address and empty status', async () => {
    await renderAsync(<EmptyCard result={{ ...BASE_RESULT, classification: 'EMPTY' }} />)
  ...
  it('shows pubkey exposed message when key is exposed', async () => {
    await renderAsync(<EmptyCard result={{ ...BASE_RESULT, classification: 'EMPTY', pubkeyExposed: true }} />)
  ...
  it('shows not exposed message when key is not exposed', async () => {
    await renderAsync(<EmptyCard result={{ ...BASE_RESULT, classification: 'EMPTY', pubkeyExposed: false }} />)
...
describe('ExposedCard', () => {
  it('renders balance and exposed heading', async () => {
    await renderAsync(<ExposedCard result={{ ...BASE_RESULT, classification: 'EXPOSED', pubkeyExposed: true }} />)
    expect(screen.getByText(/exposed/i)).toBeDefined()
    expect(screen.getByText(/50/)).toBeDefined()
  })

  it('renders P2TR-specific explanation for taproot addresses', async () => {
    await renderAsync(<ExposedCard result={{ ...BASE_RESULT, classification: 'EXPOSED', type: 'P2TR', pubkeyExposed: true }} />)
    expect(screen.getByText(/taproot/i)).toBeDefined()
  })

  it('renders HIGH_REUSE badge when flag present', async () => {
    await renderAsync(<ExposedCard result={{ ...BASE_RESULT, classification: 'EXPOSED', pubkeyExposed: true, flags: ['HIGH_REUSE'] }} />)
    expect(screen.getByText(/high reuse/i)).toBeDefined()
  })

  it('shows last spend date when available', async () => {
    await renderAsync(
      <ExposedCard result={{ ...BASE_RESULT, classification: 'EXPOSED', pubkeyExposed: true, lastSpend: '2023-01-15' }} />
    )
  ...
  it('omits USD value when null', async () => {
    await renderAsync(<ExposedCard result={{ ...BASE_RESULT, classification: 'EXPOSED', pubkeyExposed: true, balanceUsd: null }} />)
...
describe('UnresolvableCard', () => {
  it('renders address and unresolvable message', async () => {
    await renderAsync(<UnresolvableCard address="notanaddress" onRetry={() => {}} />)
...
describe('ResultCard dispatcher', () => {
  it('renders SafeAtRestCard for SAFE_AT_REST', async () => {
    await renderAsync(<ResultCard result={{ ...BASE_RESULT, classification: 'SAFE_AT_REST' }} />)
    expect(screen.getByText(/safe at rest/i)).toBeDefined()
  })

  it('renders ExposedCard for EXPOSED', async () => {
    await renderAsync(<ResultCard result={{ ...BASE_RESULT, classification: 'EXPOSED', pubkeyExposed: true }} />)
    expect(screen.getByText(/exposed/i)).toBeDefined()
  })

  it('renders EmptyCard for EMPTY', async () => {
    await renderAsync(<ResultCard result={{ ...BASE_RESULT, classification: 'EMPTY' }} />)
    expect(screen.getByText(/empty/i)).toBeDefined()
  })

  it('renders UnresolvableCard for UNRESOLVABLE', async () => {
    await renderAsync(<ResultCard result={{ ...BASE_RESULT, classification: 'UNRESOLVABLE' }} />)
...
describe('SafeAtRestCard', () => {
  it('renders safe status heading', async () => {
    await renderAsync(<SafeAtRestCard result={{ ...BASE_RESULT, classification: 'SAFE_AT_REST' }} />)
    expect(screen.getByText(/safe at rest/i)).toBeDefined()
  })
})
