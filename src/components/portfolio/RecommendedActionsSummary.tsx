...
const ACTIONS = [
  {
    ...
    description: 'These addresses have exposed public keys and non-zero balance. Move funds now.',
  ...
  },
  {
    ...
    label: 'Monitor',
    description: 'Pubkey is exposed, but balance is zero or address is low-value. Watch for incoming funds.',
  ...
  },
  {
    ...
    description: 'Safe at rest — pubkey is still hash-protected. No quantum risk today.',
  ...
  },
  {
    ...
    label: 'Manual Review',
    description: 'Address type is ambiguous (P2SH) or could not be resolved. Verify manually.',
    icon: <HelpCircle className="h-5 w-5 text-slate-400" />,
    className: '',
  },
]

export function RecommendedActionsSummary({ counts }: RecommendedActionsSummaryProps) {
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      {ACTIONS.map(action => {
        ...
        return (
          <div key={action.key} className={`flex gap-3 rounded-lg border p-4 ${action.className}`}>
